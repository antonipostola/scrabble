...
class Board:
    ...
    @staticmethod
    def get_matches(pattern: str, rack: list[str]) -> list[str]:
        ...
        regex = f"^{pattern.replace(" ", rack_tile_reg)}$"
    ...
    def get_possible_moves(self, rack: list[str]) -> list[Move]:
        ...
        def get_moves_on_line(line: str) -> list[tuple[int, str]]:
            ...
            word_starts = list(range(15))
            word_ends = list(range(15))

            for anchor in anchors:
                # cannot start between the second letter of a word and the first blank space succeeding it
                word_starts = [pos for pos in word_starts if not (anchor[0] < pos <= anchor[1]+1)]

                # cannot end between a single blank space before a word and the second to last letter of a word
                word_ends = [pos for pos in word_ends if not (anchor[0]-1 <= pos < anchor[1])]

            for i, anchor in enumerate(anchors):
                if i > 0:
                    max_left = anchors[i-1][1] + 2
                else:
                    max_left = 0
                ...
                valid_lefts = [left for left in word_starts if max_left <= left <= anchor[0]]
                valid_rights = [right for right in word_ends if right >= anchor[1]]

                for left in valid_lefts:
                    for right in valid_rights:
                        matching_words = self.get_matches(line[left:right], rack)
...
def main():
    board = Board()
    board.apply_move(Move((4, 7), "down", "beans", 0))
